    ...
    card.addEventListener('click', function(e) {
        ...
        if (miniMenu) {
            miniMenu.innerHTML = `
                ...
                <p><b>Banco:</b> ${card.classList.contains('nubank') ? 'Nubank' : card.classList.contains('bb') ? 'Banco do Brasil' : 'Caixa'}</p>
                <p><b>Número:</b> ${card.querySelector('.card-number').textContent}</p>
            `;
            // Adicione este trecho para garantir que o X funcione:
            const btnFechar = miniMenu.querySelector('.fechar-menu');
            btnFechar.addEventListener('click', function(e) {
                e.stopPropagation();
                document.querySelectorAll('.card').forEach(c => c.classList.remove('selected', 'primeiro'));
                document.querySelectorAll('.mini-menu').forEach(m => m.innerHTML = '');
...
function maskAllCardNumbers() {
    document.querySelectorAll('.card-number').forEach(cardNumber => {
        ...
        if (full) {
            ...
            const masked = groups.map((group, idx) =>
                idx === groups.length - 1 ? group : '****'
            ).join(' ');
...
document.addEventListener('click', function(e) {
    if (sidebar.classList.contains('open') && !e.target.closest('.sidebar') && !e.target.closest('.menu-icon')) {
        sidebar.classList.remove('open');
    }
});
